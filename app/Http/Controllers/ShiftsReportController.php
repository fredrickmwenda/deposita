...
class ShiftsReportController extends Controller
{
    ...
    public function ShiftsReportTwo(Request $request)
    {
        ...
        if($attendant_name == null){
            ...
            // dd('hapana');
            $attendant_jina = $attendant_name->Card_name;
        }
        // dd($attendant_jina);
        

        // dd($fromDate, $toDate, $attendant);
        ...
        if (!empty($fromDate)) {
            ...
            //$total_coins = $cashier_data->sum('coins') + $cashier_data->sum('coins.coin_amount');
            ...
            //dd('what');
            $transactions = transaction::select('*')->when($request->attendant, function ($query) use ($request) {
                return $query->where('attendant_id', $request->attendant);
            })->orderBy('id', 'desc')->get();
            $cashier_data = transaction::with(['recoveries', 'attendant'])
            ->select('*')
        ...
        //    $total_coins = $cashier_data->withSum('coins', 'coin_amount')->value('coins_sum_coin_amount');
        ...
        return view('report.periodic', compact('transactions', 'total_drop', 'total_expected', 'total_difference', 'fromDate', 'attendants', 'toDate', 'total_coins', 'total_cash', 'attendant_jina'));
    }
