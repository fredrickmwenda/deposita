$(document).ready(function() {
    ...
    $('#add-button').click(function() {
        ...
        $('#cash').val('0');
        $('#comment').val('');
        document.getElementById('difference').style.border = 'none';
    });
    ...
    $('#transaction-form').off('submit').on('submit', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // toastr.success(response.message, { fadeAway: 1000 });
                    if (response.existing_transactions && response.existing_transactions.length > 0) {
                        // Display message for existing transactions
                        toastr.info('Some transactions already exist, please check the cashier record list.', { fadeAway: 1000 });
                    } else {
                        // Display message for successful creation
                        toastr.success(response.message, { fadeAway: 1000 });
                    }
    
                } else {
                    // alert('');
                    toastr.error('Failed to save transactions.', { fadeAway: 1000 });
                }
            },
            error: function(xhr, status, error) {
                toastr.error(error, { fadeAway: 1000 });
            }
        });
    });
