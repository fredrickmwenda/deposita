...
class Patient extends CI_Controller {
	...
	public function filter_patients()
	{
		...
		// Get data
		$data = $this->patient_model->filter_patients($status);
		foreach ($data as $key => $dValue) {
			$dData[] = ['id' => $key+1,
								'name' => $dValue->firstname." ".$dValue->lastname,
								'cellphone' => $dValue->phone,
	...
	public function search3(){
		...
		$searchText = $this->input->get('search_text', true);
		$data['patients'] = $this->employee_model->search_employee($searchText);
		$data['content'] = $this->load->view('human_resources/employee_search_result0',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	...
	public function create(){
		...
		if ($this->input->post('id') == null) {
			...
			$data['patient'] = (object)$postData = [
				...
				'start_of_care' => $this->input->post('start_of_care'),
	...
	public function update(){
		...
		if ($this->form_validation->run() === true) {
			...
			$data['content'] = $this->load->view('patient/patient_edit',$data,true);
			$this->load->view('layout/main_wrapper',$data);
		}
	}
	...
	public function profile($patient_id = null){
		...
		$this->load->view('layout/patient_wrapper',$data);
	}
	...
	public function edit($patient_id = null){
		...
		$data['episodes'] = $this->episode_model->read_all_by_patient_id($patient_id);
		$data["isAdmin"] =  $this->session->userdata('isAdmin');
	...
	public function read($patient_id = null){
		$data['module'] = display("patient");
		...
		$data['branches'] = $this->main_department_model->read();
		$data['patient'] = $this->patient_model->read_by_id($patient_id);
		$data['content'] = $this->load->view('patient/patient_read',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
