...
public function index(){
  ...
  //  $this->form_validation->set_rules('user_role',display('user_type'),'required');
   ...
   $user_role = 1;
    ...
    $user_by_email = $this->dashboard_model->get_user_by_email($emailAddress);
    $user_role_by_email = $user_by_email[0]->user_role;
    $user_password_status_by_email = $user_by_email[0]->password_status;

    if ($user_role_by_email !== 1 && $user_password_status_by_email == 0 && empty($this->input->post('password'))) {

      //Log user in with a blank password (initial)
     ...
     if ($this->form_validation->run() === true) {
        ...
        if($check_user){
            ...
            $session_data=$this->session->set_userdata([
                'isLogIn'     => true,
                'password_status' => $check_user[0]->password_status,
                ...
                'gen_role'       => $check_user[0]->gen_role,
            ...
            #---------------------------------------#
             ...
             if ($check_user[0]->password_status == 0) {
               redirect('dashboard/set_password');
             }
    ...
    public function home(){
      ...
      //  $data['noPatients'] = "Test";
        ...
        if(1>0){
          ...
          $password_status = $this->employee_model->read_user_by_id($user_id)->password_status;
          $data['employeesDue'] = $this->license_model->get_employees_whose_licenses_due();

          $data['patientsDue'] = $this->episode_model->get_patients_whose_episodes_due();
          $data['password_status'] = $password_status;
    ...
    public function set_password(){
      ...
      #-------------------------------#
      $userData['userPassword'] = (object)$postData = [
        'password' 	      => md5($this->input->post('password',true)),
        'password_status' => 1,
        'user_id' => $this->session->userdata('user_id')
      ];
    ...
    public function reset_password($user_id){
      ...
      if (1>0) {
        ...
        if (!empty($postData['user_id'])) {
          ...
          redirect('human_resources/employee/form_user/'.$user_id);
        }
    ...
    public function edit(){
        ...
        if ($this->form_validation->run() === true) {
            ...
            $user_id = $this->session->userdata('user_id');
            $data['doctor'] = $this->dashboard_model->profile($user_id);
            $data['content'] = $this->load->view('profile_form_edit',$data,true);
            $this->load->view('layout/main_wrapper',$data);
        }
    }
