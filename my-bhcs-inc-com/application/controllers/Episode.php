...
class Episode extends CI_Controller {
	...
	public function patient_episodes($patientId){
		...
		#-------------------------------#
		$data['episodes'] = $this->episode_model->read_all_by_patient_id($patientId);
	...
	//	$data['lang_dprt'] = $this->main_department_model->read_lang_department();
 	...
 	public function create(){
		...
		$date = new DateTime($this->input->post('episodeToTwo',true));
		// $date->add(new DateInterval('P59D'));
		$episodeTo = $date->format('Y-m-d');
		
		// // 2021-09-30
		// return;
		$fDate = $this->input->post('episodeFrom',true);
		$fromDateArray = explode("-", $fDate);
		$fromDate = new DateTime($fromDateArray[1]."-".$fromDateArray[0]."-".$fromDateArray[2]);
		
		$episodeFrom = $fromDate->format('Y-m-d');
		// var_dump($episodeFrom);
		// var_dump($episodeTo);
		// return;
		// // 2021-08-30
		// return;
				...
				if ($this->episode_model->update($postData)) {
					...
					if (empty($this->input->post('serviceProviderId2'))) {
						...
						if($this->input->post('providerId2') != 'w' && !empty($this->input->post('serviceId2'))){
							...
							$this->service_provider_model->update($sData5);
						}
