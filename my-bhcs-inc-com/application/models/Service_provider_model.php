...
class Service_provider_model extends CI_Model {
	...
	public function read_all_by_episode_id($id = null)
	{

		return $this->db->select("lastname, firstname, service.name, service.id, service_provider.serviceId, service_provider.providerId, service_provider.id, service_provider.episodeId")
			->from('service_provider')

			->join('user', 'user.user_id = service_provider.providerId', 'left')
			->join('service', 'service.id = service_provider.serviceId', 'left')
			->where('episodeId',$id)
			->get()
			->result();
	}
