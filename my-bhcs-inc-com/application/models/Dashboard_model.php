...
class Dashboard_model extends CI_Model {
   ...
   public function userPermissionadmin($id = null)
	...
	public function notify()
	{
		...
		$patient = $this->session->userdata('isAdmin') ? $this->db->select('count(*) as total_patient')
						->from('patient')
						->get()->row() : $this->db->select('count(*) as total_patient')
										->from('patient')
										->where('branch_id', $this->session->userdata('branch'))
										->get()->row();

		$doctor = $this->session->userdata('isAdmin') ? $this->db->select('count(*) as total_doctor')
						->from('user')
						->where('gen_role', 'employee')
						->get()->row() : $this->db->select('count(*) as total_doctor')
										->from('user')
										->where('gen_role', 'employee')
