...
class Episode_model extends CI_Model {
	...
	public function get_patients_whose_episodes_due(){
			...
			return $this->db->select("firstname, lastname")
				->from($this->table)
				->where('episodeTo BETWEEN now() and date_add(now(), interval 7 day)')
				->join('user', 'user_id = patientId')
				->distinct()
				->get()
				->result();
	...
	public function read_all_by_patient_id($id = null)
	{
		return $this->db->select("*")
			->from('episode')
			->where('patientId',$id)
